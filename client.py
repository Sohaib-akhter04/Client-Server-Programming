...
def sendMsg(connection, add):
    ...
    text_connStatus.configure(text = 'STATUS: NOT Connected')
    connection.close()
...
def main(client, address):
    ...
    client.close()
    button_connect.configure(state = ACTIVE)
    text_PortIP.configure(state = NORMAL)
    input_PortIP.configure(state = NORMAL)
...
#  used to keep the client gui running ! 
window.mainloop()
